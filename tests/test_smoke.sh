...
# Image build is exercised by setup() in e2e-common.sh; this test just verifies
...
test_build() {
    ...
    local codex_rc=$?
    if [ "$codex_rc" != "0" ]; then
        log ERROR "codex --version via entrypoint exited $codex_rc"
        log ERROR "output: ${out:0:500}"
        return 1
    fi
...
test_smoke_codex() {
    ...
    out=$(run_codex "$prompt" 2>&1) || {
        log ERROR "codex exited non-zero"
        log ERROR "output: ${out:0:1000}"
        return 1
    }
